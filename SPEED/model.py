...
class SPEED_model(nn.Module):
    ...
    """
    def __init__(self, cell_num,peak_num, h_peak_features, h_cell_features, emb_features, peak_batch_norm, peak_layer_norm, cell_batch_norm, cell_layer_norm,dropout_p=[0.4,0.4],eps=1e-8,weight1=None,weight0=None,cell_depth_mode=1,is_spatial=False,peak_train=True):
        ...
        if is_spatial:
            self.ImgEncoder = Encoder(in_features=2048, h_features=[512,128], z_features=emb_features, batch_norm=cell_batch_norm, layer_norm=cell_layer_norm,dropout_p=dropout_p[1])
    ...
    def bce_loss_autoweight(self, inputs, target,reduction):
        weight = self.weight1*target+self.weight0*(1-target)
        loss = torch.nn.functional.binary_cross_entropy(inputs, target, weight=weight, reduction=reduction)
        return loss
    ...
    def pcc_loss(self, depth):
        return torch.corrcoef(torch.cat([self.emb[:,:-1],depth.reshape(-1,1)],dim=1).T)[:,-1].abs().mean()
    
    def loss(self, x, x_re, peak,method='cosine',reduction='mean',alpha=10,beta=1):
        if method=='wBCE':
            loss_rec = self.bce_loss_autoweight(x_re, x,reduction=reduction)
        elif method=='cosine':
            ...
            loss_rec = F.binary_cross_entropy(x_re, x, reduction=reduction)
        elif method=='both':
            l_p_1 = F.cosine_embedding_loss(x_re, x,torch.ones([x.shape[0]]).to(x.device),reduction=reduction)
            l_p_2 = F.cosine_embedding_loss(x_re.T, x.T,torch.ones([x.shape[1]]).to(x.device),reduction=reduction)
            l_p_3 = self.bce_loss_autoweight(x_re, x,reduction=reduction)
            loss_rec = ((l_p_1+l_p_2)/2+l_p_3)/2
        else:
            raise ValueError('method should be one of ["wBCE","cosine","BCE","both"]!')
