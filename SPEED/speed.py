...
class SPEED:
    ...
    def __init__(
        ...
        - is_spatial (bool, optional): Whether to incorporate spatial information. ``False`` in stage 1 and ``True`` in stage 2. Default is False.
        - image (array-like, optional): The image data in RGB format in stage 2. If ``None``, image information will not be used. This parameter is used if ``is_spatial=True``.
        - k_degree (int, optional): Degree of spatial neighbor used for spatial relative position encoding in stage 2. Default is 5. This parameter is used if ``is_spatial=True``.
        - adata_sc (AnnData, optional): Reference single-cell epigenomics data in stage 2. This parameter is used if ``is_spatial=True``.
        - peak_name (str, optional): Name of the peak embedding variable in `adata_sc`. Default is 'peak_SPEED'. This parameter is used if ``adata_sc`` is not None.
        """
        ...
        if is_spatial:
            if adata_sc is None:
                ...
                self.is_ref = True
            self.coo_mtx = find_pairwise_dis_matrix(adata.obsm['spatial'],self.k_degree)
            self.coo_mtx[self.coo_mtx!=0] = 1+np.log(self.coo_mtx[self.coo_mtx!=0])
            if image is not None:
                self.img = Encode_image_resnet(image, adata.obsm['spatial'])
                self.is_image = True
            else:
    ...
    def setup_data(
        ...
        - dense (bool, optional): Whether to use dense matrix format. 
    ...
    def train(
        ...
        - alpha (float, optional): Weight of constraint on the similarity between the peak embedding of spatial data and single-cell data in stage 2. Default is 10.
        ...
        - epo_max (int, optional): Maximum number of epochs without improvement before early stopping. Default is 30.
        ...
        - method (str, optional): Loss function method. Should be one of ['cosine', 'BCE', 'wBCE', 'both']. Default is 'cosine'.
        ...
        - save_path (str, optional): Path to save the model if ``save=True``.
        """
        ...
        for epoch in range(1,epoch_num):
            ...
            for row in tqdm(self.trainset_row):
                ...
                if self.is_spatial:
                    ...
                    coo = torch.Tensor(self.coo_train[row,:]).to(device)
                else:
                    img, coo = None, None
                ...
                for col in self.trainset_col:
                    ...
                    if self.is_spatial is False:
                        loss = model.loss(label, x_re,peak=None, method=method,reduction=reduction,alpha=alpha,beta=beta)
                    else:
                        p = torch.Tensor(self.peak_embedding_train[col,:])
                        p = p.to(device)
                        loss, emb_loss, img_loss = model.loss(label, x_re,peak=p, method=method,reduction=reduction,alpha=alpha,beta=beta)
            ...
            if epoch % val_epoch == 0:
                ...
                with torch.no_grad():
                    ...
                    for row in tqdm(self.valset_row):
                        ...
                        if self.is_spatial:
                            ...
                            coo = torch.Tensor(self.coo_val[row,:]).to(device)
                        else:
                            img, coo = None, None
                        if self.dense is False:
                            y = y.to_sparse_coo()
                    ...
                    if val_loss_total > loss_before:
                        ...
                        if save:
                            save_path_file = f'{save_path}/model_'+str(epoch)+'.pth'
                ...
                print ("Epoch[{}/{}], Loss: {:.5f}, emb Loss: {:.2f}, img Loss: {:.2f}, Val Loss: {:.5f}, Val emb Loss: {:.2f}, Val img Loss: {:.2f}" 
                       .format(epoch, epoch_num, loss_record['loss'][-1], loss_record['emb_loss'][-1], loss_record['img_loss'][-1], loss_record['val_loss'][-1], loss_record['val_emb_loss'][-1], loss_record['val_img_loss'][-1]))
                if epo_conv == epo_max:
                    print('convinient')
    ...
    def get_coo_embedding(self, device='cpu', return_np=True):
        """
        ...
        model.eval()
        with torch.no_grad():
            emb = model.CooEncoder(torch.Tensor(self.coo_mtx).to(device))
            self.coo_embedding = emb.cpu().detach().numpy()
        if return_np: return self.coo_embedding
